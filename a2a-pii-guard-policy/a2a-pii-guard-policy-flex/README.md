# "a2a-pii-guard-policy" Policy

This is the policy-implementation half of the **A2A PII Guard** policy. It compiles to a `wasm32-wasip1` module loaded by Flex Gateway at runtime.
...
### Build asset files
The `make build-asset-files` goal generates all the policy asset files required to build, execute, and publish the policy.

> **Note:** Unlike the default PDK template, `src/generated/config.rs` is hand-maintained for this policy because the GCL schema uses nested object arrays (`predefinedEntities[]`, `customEntities[]`) that the current `cargo anypoint config-gen` cannot prettify. The hand-rolled struct preserves the schema and is checked in. The `build-asset-files` target only fetches the latest definition from Exchange and verifies the file is present — it does not regenerate it. Run `cargo anypoint config-gen` manually only when adding top-level scalar properties; do **not** regenerate over the nested arrays.
...
- An `api.yaml` file updated with the desired policy configuration. The repository ships with an example combining predefined and custom entities.

The `playground/config` directory can also contain other resource definitions, such as accessory services used by the policy.
...
Integration tests require a `tests/config/registration.yaml` produced via `anypoint-cli-v4 registration create-local` (it is gitignored).
...
definition_asset_id = { name = "a-two-a-pii-guard", version = "1.1.0-DEV" }
```
...
[package.metadata.anypoint]
group_id = "00000000-0000-0000-0000-000000000000"
definition_asset_id = { name = "a-two-a-pii-guard", version = "1.1.0" }
```
